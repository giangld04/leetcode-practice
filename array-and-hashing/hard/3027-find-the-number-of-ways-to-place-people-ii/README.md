...
2. Now consider two points upper-left corner points[i] and lower-right corner points[j], such that i  and points[i][0]  and points[i][1] >= points[j][1].
3. Instead of brute force looping, we can save the largest y-coordinate that is no larger than points[i][1] when looping on j, say the value is m. And if m , the upper-left and lower-right corner pair is valid.
