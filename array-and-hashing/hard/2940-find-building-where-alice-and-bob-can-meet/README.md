...
2. For each query [x, y], if x == y or heights[x] , then the answer is y since x ≤ y.
3. Otherwise, we need to find the smallest index t such that y  and heights[x] . Note that heights[y] , so heights[x]  is a sufficient condition.
